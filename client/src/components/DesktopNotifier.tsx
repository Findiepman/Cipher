 ...
 *
 * Raising one goes through the platform adapter rather than the Notification
 * API directly: in a browser that is the same thing, in the desktop app it
 * is the shell handing the text to the OS, because the WebView there has no
 * notification permission of its own to grant.
 */
...
export function DesktopNotifier() {
  ...
  useEffect(() => {
    const clear = () => {
      if (document.hasFocus() && !document.hidden) platform.dismissNotifications();
    };
